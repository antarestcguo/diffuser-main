...
class AttentionLayout(nn.Module):
    r"""
    A cross attention layer.
    ...
    def __init__(
        ...
        scale_qk: bool = True,
        only_cross_attention: bool = False,
        processor: Optional["AttnProcessorLayout"] = None,
    ):
        super().__init__()
        inner_dim = dim_head * heads
        cross_attention_dim = cross_attention_dim if cross_attention_dim is not None else query_dim
        ...
        if processor is None:
            # processor = (
            #     AttnProcessor2_0() if hasattr(F, "scaled_dot_product_attention") and scale_qk else AttnProcessorLayout()
            # )
            processor = (
                AttnProcessorLayout()
            )
    ...
    def set_processor(self, processor: "AttnProcessorLayout"):
        # if current processor is in `self._modules` and if passed `processor` is not, we need to
        # pop `processor` from `self._modules`
        if (
            hasattr(self, "processor")
...
class AttnProcessorLayout:
    def __call__(
        self,
        attn: AttentionLayout,
        hidden_states,
        encoder_hidden_states=None,
        attention_mask=None,
    ):
