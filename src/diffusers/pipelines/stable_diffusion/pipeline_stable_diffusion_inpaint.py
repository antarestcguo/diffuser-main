...
class StableDiffusionInpaintPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def blended_edit(
        ...
        image = image.transpose(0, 3, 1, 2)
        image = torch.from_numpy(image).to(dtype=torch.float32) / 127.5 - 1.0
        image_latents = self.vae.encode(image.to(dtype=torch.float16).to(self.vae.device)).latent_dist.sample(
            generator=generator)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_source_latents = self.scheduler.add_noise(
                    image_latents, torch.randn_like(latents), t
                )
                # hard reset
                if i == len(timesteps):
                    latents = latents * mask[0][None] + image_latents * (1 - mask[0][None])
                else:
                    latents = latents * mask[0][None] + noise_source_latents * (1 - mask[0][None])
