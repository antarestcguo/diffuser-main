...
from diffusers.datasets import TCCustomDiffusionFixedMaskInpaintingDataset
...
def collate_fn(examples, with_prior_preservation):
    ######################### copy from train_dreambooth_inpainting
    input_ids = [example["instance_prompt_ids"] for example in examples]
    pixel_values = [example["instance_images"] for example in examples]
    mask_values = [example["mask"] for example in examples]
    masked_values = [example["masked_image"] for example in examples]

    pixel_values = torch.stack(pixel_values)
    pixel_values = pixel_values.to(memory_format=torch.contiguous_format).float()
    masked_values = torch.stack(masked_values)
    masked_values = masked_values.to(memory_format=torch.contiguous_format).float()
    mask_values = torch.stack(mask_values)
    mask_values - mask_values.to(memory_format=torch.contiguous_format).float()
    input_ids = torch.cat(input_ids, dim=0)

    batch = {"input_ids": input_ids, "pixel_values": pixel_values, "masks": mask_values, "masked_images": masked_values}
...
def main(args):
    ...
    train_dataset = TCCustomDiffusionFixedMaskInpaintingDataset(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                latent_model_input = torch.cat([noisy_latents, mask, masked_latents],
                                               dim=1)  # unet need to change the input channel number
