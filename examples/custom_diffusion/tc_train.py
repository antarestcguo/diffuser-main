...
from diffusers.datasets import TCCustomDiffusionDataset
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--token_path", type=str,
        default='',
        help="Path to token_path for attn and textembedding.bin files",
    )
    # parser.add_argument(
    #     "--attn_path", type=str,
    #     default='',
    #     help="Path to attn_path for attn and attn.pth files",
    # )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="image list of the instance file, contains the caption.",
    )
    parser.add_argument(
        "--other_file_name",
        type=str,
        default=None,
        help="image list of the other file, contains the caption.",
    )
    parser.add_argument(
        "--b_other", action="store_true",
        help="if use other file",
    )
    parser.add_argument(
        "--caption_flag_str", type=str, default="ori_caption",
        help="option [ori_caption,one_caption]",
    )
    parser.add_argument(
        "--ft_type", type=str, default="text",
        help="option [text, unet,attn,attn+text,unet+text]",
    )
    # parser.add_argument(
    #     "--instance_prompt",
    #     type=str,
...
def main(args):
    ...
    # modify by tc, check load from file
    ...
    for modifier_token, initializer_token in zip(
        ...
        # check if the token in the vocab , add by tc
        vocab = tokenizer.get_vocab()
        print("original vocab len:", len(tokenizer))
        if modifier_token in vocab:
            print("modify token:", modifier_token, "already in vocab")
            continue

        # Add the placeholder token in tokenizer
        num_added_tokens = tokenizer.add_tokens(modifier_token)
        print("add new word,", modifier_token, "vocab len:", len(tokenizer))
        ...
        print("modify token:", modifier_token, "init token:", initializer_token, "init_token_id:", token_ids)
        # Check if initializer_token is a single token or a sequence of tokens
        if len(token_ids) > 1:
            raise ValueError("The initializer token must be a single token.")
        initializer_token_id.append(token_ids[0])
        modifier_token_id.append(tokenizer.convert_tokens_to_ids(modifier_token))
        modify_token_list.append(modifier_token)
        print("new modifier_token and id:", modifier_token, tokenizer.convert_tokens_to_ids(modifier_token))
        print("init id:", token_ids)
    # Resize the token embeddings as we are adding new special tokens to the tokenizer
    text_encoder.resize_token_embeddings(len(tokenizer))
    # Initialise the newly added placeholder token with the embeddings of the initializer token
    token_embeds = text_encoder.get_input_embeddings().weight.data
    for modifier_token, x, y in zip(modify_token_list, modifier_token_id, initializer_token_id):
        file_name = os.path.join(args.token_path, f"{modifier_token}.bin")
        if os.path.exists(file_name):
            state_dict = torch.load(file_name, map_location="cpu")
            loaded_token, embedding = next(iter(state_dict.items()))
            embedding = embedding.to(dtype=text_encoder.dtype, device=text_encoder.device)
            print(modifier_token, "load from file:", file_name)
        else:
            embedding = token_embeds[y]
            print(modifier_token, "init from id:", y)

        token_embeds[x] = embedding
    ...
    # add by tc, load attn
    # file_name = os.path.join(args.pretrained_model_name_or_path,"pytorch_custom_diffusion_weights.bin")
    # if os.path.exists(file_name):
    #     unet.load_attn_procs(args.pretrained_model_name_or_path, weight_name="pytorch_custom_diffusion_weights.bin")

    # add by tc
    placeholder_token = " ".join(args.modifier_token)
    ########################################################
    ########################################################

    vae.requires_grad_(False)
    # if args.modifier_token is None:
    if args.ft_type.find("text") == -1:
    ...
    train_dataset = TCCustomDiffusionDataset(
        instance_dir=args.instance_dir,
        other_dir=args.other_dir,
        instance_file_name=args.instance_file_name,
        other_file_name=args.other_file_name,
        caption_flag_str=args.caption_flag_str,
    ...
    if args.ft_type == 'unet':
        unet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            unet, optimizer, train_dataloader, lr_scheduler
        )
    if args.ft_type == 'unet+text':
