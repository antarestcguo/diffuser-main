from diffusers import StableDiffusionPipeline, StableDiffusionStepPipeline, StableDiffusionImg2ImgPipeline, \
    StableDiffusionImg2ImgStepPipeline, StableDiffusionImg2ImgMergePipeline
import os
...
pipeline = StableDiffusionImg2ImgMergePipeline.from_pretrained(
    config.model_path
)
pipeline.to("cuda")
...
for i in range(config.gen_num):
    img = pipeline(
        image=[Image.open(config.input_img_name_1), Image.open(config.input_img_name_2)],
